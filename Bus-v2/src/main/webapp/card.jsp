...
<title>Cartões - StackOverbus</title>
</head>
<body>
		...
		<div class="center col-lg-10 col-sm-12">
			...
			<form action="ControllerServlet" method="post">
				<div class="row">
					...
					<div class="col-12 col-lg-3">
						<div class="mb-2">
							<label for="user">Nome Titular</label> 
							<input type="date" name="user" id="user"
								class="form-control">
						</div>
					</div>  
					<div class="col-12 col-lg-3 mt-4">
						<button type="submit" class="btn btn-primary" name="action" value="searchCard">Filtrar</button>
					</div>  
				</div>
			</form>
			<c:choose>
				<c:when test="${fn:length(userCars) > 0}">
					<table class="table table-responsive table-striped table-hover" >
						...
						<c:forEach var="card" items="${userCards}" varStatus="index">
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${card.type == 'CORRIDA'}">
											<img src="img/escolar_icon.png" alt="Escolar">
										</c:when>
										<c:when test="${card.type == 'CAMINHADA'}">
											<img src="img/empresarial.png" alt="Empresarial">
										</c:when>
										<c:when test="${card.type == 'CICLISMO'}">
											<img src="img/padrao_icon.png" alt="Padrao">
										</c:when>
									</c:choose>
								</td>
								<td>${card.saldo}</td>
				...
				<c:otherwise>
					<c:out value="Sem nenhum cartão registrado."></c:out>
				</c:otherwise>
			</c:choose>
		</div>
	</div>
